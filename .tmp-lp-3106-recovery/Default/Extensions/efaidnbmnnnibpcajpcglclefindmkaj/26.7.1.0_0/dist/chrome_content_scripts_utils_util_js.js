...
/**
 * Generates a unique ID (UUID v4). Falls back to a random string if an error occurs.
 */
...
/**
 * Builds the conversion URL and dispatches execute-direct-verb, analytics, and akamai-ping.
 * The caller is responsible for obtaining tabId (and frameId/targetSectionId when needed).
 *
 * @param {Object} options
 * @param {string} options.tabId - Tab ID returned by the get-tab-id service worker message.
 ...
 * @param {string} [options.frameId] - Frame ID for iframe-based whole-page conversion.
 * @returns {void}
 */
var executeHtmlToPdfConversion = function executeHtmlToPdfConversion(_ref4) {
  var tabId = _ref4.tabId,
    fileName = _ref4.fileName,
    source = _ref4.source,
    workflow = _ref4.workflow,
    targetSectionId = _ref4.targetSectionId,
    element = _ref4.element,
    frameId = _ref4.frameId;
  var originalUrl = window.location.href;
  var fileUrl = "https://convert-pdf-webpage/?tabId=".concat(tabId) + "&tabOriginalUrl=".concat(encodeURIComponent(originalUrl));
  if (targetSectionId) {
    // Only store if element is provided — callers that pre-stored the clone must not push a dead entry.
    if (element) storeTargetElementForConversion(targetSectionId, element);
    fileUrl += "&targetSectionId=".concat(encodeURIComponent(targetSectionId));
  } else if (frameId) {
    fileUrl += "&frameId=".concat(frameId);
  }
